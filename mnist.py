...
def load_dataset():
    ...
    # We reserve the last 500 training examples for validation.
...
class NormalisedGeometricMeanLayer(lasagne.layers.ElemwiseMergeLayer):
    def __init__(self, incomings, **kwargs):
        super(NormalisedGeometricMeanLayer, self).__init__(
            incomings, T.add, **kwargs
        )
        self.M = len(incomings)

    def get_output_for(self, incomings, **kwargs):
        geo_mean = T.exp(
            super(NormalisedGeometricMeanLayer, self).get_output_for(
                [T.log(incoming) / self.M for incoming in incomings],
                **kwargs
            )
        )
        Z = T.sum(geo_mean, axis=1)[:, np.newaxis]
        return geo_mean / Z
...
def build_cnn(input_var=None, num_modules=8):
    ...
    network['input'] = lasagne.layers.InputLayer(
        shape=(None, 1, 28, 28),
        input_var=input_var
    )
    ...
    network['pool1'] = lasagne.layers.MaxPool2DLayer(
        network['conv1'], pool_size=(2, 2)
    )
    ...
    network['pool2'] = lasagne.layers.MaxPool2DLayer(
        network['conv2'], pool_size=(2, 2)
    )
    ...
    for i in range(1, num_modules + 1):
        ...
        outputs.append(network['prob_{}'.format(i)])

    network['prob'] = NormalisedGeometricMeanLayer(
        outputs
    )
...
def main(args):
    ...
    test_prediction = lasagne.layers.get_output(
        network['prob'], deterministic=True
    )
